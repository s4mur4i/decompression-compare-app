...
function App() {
  ...
  useEffect(() => {
    ...
    if (descent) setDescentRate(Math.max(1, Number(descent) || 9));
    if (algo) setAlgorithm(algo);
    if (o2) setFO2(Math.min(1, Math.max(0.16, Number(o2) / 100)));
    if (gfl) setGfLow(Number(gfl) || 30);
  ...
  useEffect(() => {
    ...
    if (gfLow !== 30) params.set('gfl', gfLow);
    if (gfHigh !== 70) params.set('gfh', gfHigh);
    const newUrl = `${window.location.pathname}?${params.toString()}`;
    window.history.replaceState(null, '', newUrl);
  }, [stops, descentRate, algorithm, fO2, gfLow, gfHigh, initialized]);

  const result = useMemo(() => {
    if (stops.length === 0) return null;

    const baseProfile = calculateDiveProfile(stops, descentRate);
    let decoInfo = null;

    if (algorithm === 'buhlmann') {
      decoInfo = calculateBuhlmann(baseProfile.phases, fO2, gfLow, gfHigh, descentRate);
      const fullProfile = addAscentPhases(baseProfile, decoInfo.decoStops, descentRate);
      return { ...fullProfile, decoInfo };
    } else {
      const fullProfile = simpleAscent(baseProfile, descentRate);
      return { ...fullProfile, decoInfo };
    }
  }, [stops, descentRate, algorithm, fO2, gfLow, gfHigh]);
